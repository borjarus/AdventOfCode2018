...
and E is closest to 17 (both including the coordinate's location itself). Therefore, in this example, 
...
is less than 32, that location is within the desired region. Using the same coordinates as above, 
...
8, 9\n"
        ...
        let largestAreaSize =
            let groups = 
                coordinates
                |> Seq.choose (closestTo parsedInput)
                |> Seq.groupBy fst 
            groups
            |> Seq.map snd
            |> Seq.choose (fun el -> 
                let second = el |> Seq.map  snd
                if not (isInfiniteArea second)
                then Some (Seq.length el)
                else None
            )
            |> Seq.reduce max
...
8, 9\n"
    ...
    let part1() = 
        let input = readLinesFromFile(@"day6.txt")
        let parsedInput = 
            input
            |> Seq.map parseLine
