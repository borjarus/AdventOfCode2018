...
be entirely within the 300x300 grid. Identify this square using the X,Y coordinate of its top-left fuel cell. For example:
...
open Helpers
    ...
    let power x y serial = 
        let rake = (x + 10)
        (rake * y) + serial
        |> (*) rake
        |> (fun x -> (x / 100) % 10 - 5)

    let buildGrid n serial = List.init n (fun y -> List.init n (fun x -> power (x + 1) (y + 1) serial))
    ...
    let getHighestPowerSquare size grid =
        ...
        List.init (len - size + 1) (fun x -> List.init (len - size + 1) (fun y -> (x + 1), (y + 1), getSquarePowerSum x y size grid))
        |> List.collect id
    ...
    let part1() =        
        ...
        let x, y, _ = input |> buildGrid 300 |> getHighestPowerSquare 3
        sprintf "%d,%d" x y
    ...
    let part2() = 
        let input =  int "6878"
        let x, y, size, _ = input |> buildGrid 300 |> getHighestPowerSquareWithSize
        sprintf "%d,%d,%d" x y size